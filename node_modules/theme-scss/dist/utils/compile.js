...
function compileSCSS(target) {
    try {
        ...
        if (!target.endsWith('.scss')) {
            logger.notices.error(`${logger.dye.yellow(target)} does not end with a .scss extension. Check your theme-config and try again.`, true);
        }
...
export function developmentCompile() {
    ...
    if (!paths.dev.output.endsWith('.css')) {
        logger.notices.error(`${logger.dye.yellow(paths.dev.output)} does not end with a .css extension. Check your theme-config and try again.`, true);
    }
    writeFileSync(paths.dev.output, (themeConfig.dev?.mod ? generateBetterDiscordMeta() + '\n\n' : '') + compiledSCSS);
    logger.notices.success(`Successfully recompiled, took ${logger.dye.yellow(`${Date.now() - start}ms`)}`);
}
/**
...
export function productionCompile() {
    ...
    try {
        ...
        if (!existsSync(outDir)) {
            logger.notices.warning('Source output folder does not exist, so it was created for you.');
            mkdirSync(outDir, { recursive: true });
        }
        if (!paths.dist.output.endsWith('.css')) {
            logger.notices.error('Your dist output path does not end with a .css extension. Check your theme-config and try again.', true);
        }
            ...
            for (const platform of platforms) {
                ...
                switch (platform) {
                    case 'betterdiscord':
                        ...
                        writeFileSync(path.join(paths.dist.clients, `${themeName}.theme.css`), generateBetterDiscordMeta() +
                            '\n\n' +
                    ...
                    case 'userstyle':
                        ...
                        writeFileSync(path.join(paths.dist.clients, `${themeName}.user.css`), `@-moz-document domain("discord.com") {\n` +
        ...
        logger.notices.success(`Successfully compiled source files for distribution, took ${logger.dye.yellow(`${Date.now() - start}ms`)}.`);
    }
    catch (err) {
        console.log(err);
