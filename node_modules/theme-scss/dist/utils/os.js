    ...
    switch (process.platform) {
        ...
        default:
            if (!themeConfig.dev?.output) {
                logger.notices.error(`${process.platform} is not supported for development. Please provide a custom output path for development instead.`);
            }
            return null;
    }
}
 ...
 */
export function getThemeFolder(mod) {
    // A path was specificed so we use that instead.
    if (themeConfig.dev?.output)
        return themeConfig.dev?.output;
    let file;
    switch (mod) {
        case 'betterdiscord':
            if (OS === 'win32') {
                ...
                file = path.resolve(process.env.HOME, 'Library', 'Application Support', 'BetterDiscord', 'themes');
            }
            else if (OS === 'linux') {
        ...
        case 'vencord':
            if (OS === 'win32') {
                ...
                file = path.resolve(process.env.HOME, 'Library', 'Application Support', 'Vencord', 'themes');
            }
            else if (OS === 'linux') {
            ...
            break;
        default:
            logger.notices.error(`${mod} is not supported for development. Please provide a custom path instead.`, true);
            return;
    }
