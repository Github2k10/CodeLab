...
function getConfig() {
    try {
        ...
        if (err.code === 'ENOENT') {
            logger.notices.error(`Unable to locate ${logger.dye.yellow(configPath)}. Double check that it exists.`, true);
            return null;
        }
...
function defaultInfo() {
    ...
    const defaults = { name, author, description, version };
    ...
    if (missing.length) {
        try {
            ...
            if (missing.length) {
                logger.notices.error(`You are missing some meta values (${missing.join(', ')}). There was an attempt to use your ${logger.dye.yellow('package.json')} file, but it was missing these values as well.`, true);
            }
        }
        catch (err) {
            console.log(err);
            logger.notices.error(`You are missing some meta values (${missing.join(', ')}). There was an attempt to use your ${logger.dye.yellow('package.json')} file, but it appears to not exist.`, true);
        }
    }
    return defaults;
}
...
export const themeImport = themeConfig.import ||
    `https://${defaults.author.toLowerCase().replaceAll(' ', '-')}.github.io/${themeName}/${defaults.dist.output}`;
/**
 * Path file locations for files.
 */
export const paths = {
    themeMetas: {
        betterdiscord: path.join(process.cwd(), themeConfig?.metas?.betterdiscord || 'metas/betterdiscord.json'),
